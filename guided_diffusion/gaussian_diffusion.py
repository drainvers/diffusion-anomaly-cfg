...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        :param clf_free: If using classifier free guidance, supply the output of 
            CFG conditioning, otherwise, leave as None
    ...
    def condition_clf_free(self, model, x, t, guidance_scale, model_kwargs=None):
        """
        Classifier-free guidance
        """
        model_kwargs["clf_free"] = True
        model_kwargs["p_uncond"] = -1 # Disable random label dropping for inference
        eps_cond = model(x, self._scale_timesteps(t), **model_kwargs)

        # Null embedding
        uncond_kwargs = model_kwargs.copy()
        uncond_kwargs["null"] = True # Drop labels for unconditional sampling
        eps_uncond = model(x, self._scale_timesteps(t), **uncond_kwargs)
    ...
    def p_sample_loop(
        ...
        :param guidance_scale: Guidance scale for CFG (-1 if unused.)
    ...
    def p_sample(
        ...
        clf_free = (
            functools.partial(
                self.condition_clf_free, model=model, guidance_scale=guidance_scale, model_kwargs=model_kwargs
            )
            if guidance_scale != -1
            else None
        )
        ...
        if cond_fn is not None and guidance_scale == -1: # Use classifier guidance
    ...
    def ddim_sample(
        ...
        if cond_fn is not None and guidance_scale == -1: # Use classifier guidance
            cond_kwargs = dict(y=model_kwargs['y'])
            out, saliency = self.condition_score(cond_fn, out, x, t, model_kwargs=cond_kwargs)
        else:
    ...
    def training_losses(self, model,  x_start, t, model_kwargs=None, noise=None):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        elif 'y' not in model_kwargs.keys():
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start)
    ...
    def calc_pred_xstart_loop(
        ...
        :param model: the model to evaluate loss on.
        :param use_ddim: if True, use ddim (deterministic) encoding else ddpm stochastic encoding.
        :param clip_denoised: if True, clip denoised samples.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.

        :return: a dict containing the following keys:
                 - 'xstart': a tensor of shape [N, C, H, W] of predicted x_0 guided to healthy.
                 - 'xstart_null': a tensor of shape [N, C, H, W] of predicted x_0 for unguided.
        """
        ...
        timesteps = noise_level if noise_level else self.num_timesteps

        x_t = None
        for t in list(range(timesteps - 1)):
